...
namespace comm
{
    ...
    static const char* get_pinyin_by_code(uint32_t code);
    ...
    unsigned get_pinyin(const char* chs_ptr, char* buf, const unsigned buf_len, EPinyinMode mode)
    {
        ...
        uint32_t code, i, j;
        const uint32_t chineseLen = strlen(chs_ptr);
        for (i = 0; i < chineseLen; ++i)
        {
            ...
            ucHigh = (uint8_t)chs_ptr[i];
            ucLow = (uint8_t)chs_ptr[++i];
            ...
            const char* pBuf = get_pinyin_by_code(code);
            if (NULL != buf)
            {
                for (j = 0; j < strlen(pBuf); ++j)
                {
                    ...
                    switch (mode)
                    {
                        case EPinyinModeAllUpper:break;
                        case EPinyinModeAllLower:cc = tolower(cc); break;
                        case EPinyinModeFirstUpper:if (j != 0)cc = tolower(cc); break;
                    }
            ...
            {
                real_len += strlen(pBuf);
            }
        }

        return real_len;
    }
    const char* get_pinyin_by_code(uint32_t code)
    {
        switch (code)
        {
            case 6325:
    ...
    std::string get_first_letter(const std::string& chs_str)
    {
        ...
        std::string result;
        int H, L, W;
        for (unsigned i = 0; i < chs_str.size(); i++)
        {
            H = (unsigned char)(chs_str[i + 0]);
            L = (unsigned char)(chs_str[i + 1]);

            if (H < 0xA1 || L < 0xA1)
            {
                result.push_back(chs_str[i]);
                continue;
            }
            else
            {
            ...
            if (W > 1600 && W < 5590)
            {
                for (int j = 22; j >= 0; j--)
                {
                    ...
                    {
                        result.append(lc_FirstLetter[j]);
                        i++;
                        break;
                    }
                }
            ...
            else 
            {
                ...
                if (W >= 0 && W <= 3007)
                {
                    result.push_back(ls_SecondSecTable[W]);
                }  
                else
                {
                    result.push_back((char)H);
                    result.push_back((char)L);
                }
            }
        }
